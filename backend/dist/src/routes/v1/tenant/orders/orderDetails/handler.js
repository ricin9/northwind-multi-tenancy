...
export const orderDetailsGroup = new OpenAPIHono().openapi(list, async (c)=>{
    ...
    const result = await db.transaction(async (tx)=>{
        ...
        const existingOrderDetail = await tx.query.orderDetails.findFirst({
            ...
            where: and(eq(orderDetails.productId, detail.productId), eq(orderDetails.orderId, orderId))
        });
        const quantityDiff = existingOrderDetail ? (detail.quantity || 1) - existingOrderDetail.quantity : detail.quantity || 1;
        ...
        const { lastInsertRowid } = await tx.insert(orderDetails).values(detail).onConflictDoUpdate({
            target: [
    ...
    await db.transaction(async (tx)=>{
        ...
        await tx.delete(orderDetails).where(and(eq(orderDetails.orderId, orderId), eq(orderDetails.productId, productId)));
        await tx.update(products).set({
